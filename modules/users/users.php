...
function GetUsersMoneyOut() {
	...
	if (isset($_SESSION['Data']["sendbtn"])) { $P=$_SESSION['Data'];
		...
		$m=(int)$P["sumout"]; $p=$P["phoneout"]; $p=preg_replace('/[^0-9_\.\-]+/i', '', $p); $w=$P["webmoneyout"]; $w=preg_replace('/[^0-9_\.\-]+/i', '', $w); $s=$P["sberout"]; $s=preg_replace('/[^0-9_\.\-]+/i', '', $s);
		if ($m==0 || $m>$balance) { $_SESSION["msg"]='<div class="ErrorDiv">Неверно введена сумма вывода</div>'; SD(); @header("location: /users/pay"); exit(); } else {
		if ($m<500) { $_SESSION["msg"]='<div class="ErrorDiv">Минимальная сумма для вывода составляет 500 рублей</div>'; SD(); @header("location: /users/pay"); exit(); } else {
		if ($p=="" && $w=="" && $s=="") { $_SESSION["msg"]='<div class="ErrorDiv">Необходимо ввести данные для удобного способа вывода</div>'; SD(); @header("location: /users/pay"); exit(); } else {
			$tx="Заявка на вывод денег:\r\n"; IF ($p!="") { $tx.="На счет телефона: ".$p."\r\n"; } IF ($w!="") { $tx.="На кошелек webmoney R: ".$w."\r\n"; } IF ($s!="") { $tx.="На карту сбербанка: ".$s."\r\n"; } $tx.="<b>Статус: ожидает проверки администрацией</b>";
			DB("INSERT INTO `_userspays` (`data`,`money`,`text`,`uid`,`pid`) VALUES ('".time()."','-".$m."','".$tx."','".(int)$USER["id"]."','0')"); $_SESSION["msg"]='<div class="SuccessDiv">Спаксибо! Заявка на вывод денег успешно создана.</div>'; SD(); @header("location: /users/pay"); exit(); 
		}}}
	}
	// ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- ---
	...
	if ($balance<500) { $text="Вывод денег доступен при балансе более 500 рублей"; } else {
		...
		$text.="<span style='color:red;'>Внимание!</span> Вывод денег производится 3 раза в неделю, статус заявки и информацию о завершении вывода денег вы увидите в таблице ниже.".$C10;
		...
		$text.="Заполните поле ниже соответственно более удобному для вас методу вывода денег, если вы заполняете несколько полей, метод вывода будет выбран администрацией Bubr.ru:".$C15;
	...
	$mon.="<td class='act2'><data>".$d[4]."</data></td><td class='act2'><data>".$ar["money"]." руб.</data></td><td class='name'>".nl2br($ar["text"])."</td></tr>"; endfor; $mon.="</table></div>"; $text.=$mon; }
	// ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- --- ---- ---
...
function GetUsersList() {
	...
	if ($data["total"]==0) { $articlesold='Здесь нет статей'; } else { $articlesold="<table>"; for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $articlesold.="<tr>"; $d=ToRusData($ar["data"]);
		$articlesold.="<td class='name'><a href='/post/view/".$ar["id"]."' target='_blank'>".$ar["name"]."</a><br><data>ID=".$ar["id"]." / Одобрено: $d[0] / Статус: заработок остановлен</data></td>"; $articlesold.="<td class='act2'>Показов: ".$ar["seen"]."</td>";
...
function GetUsersArticle() {
	...
	$p=DB("SELECT * FROM `_widget_pics` WHERE (`pid`='".(int)$dir[2]."' && `link`='userslenta' && `point`='report' && `stat`=1) order by `id` ASC"); $report=''; $marker=1; 
