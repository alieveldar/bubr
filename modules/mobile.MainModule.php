...
$PHPmodules = array(
	...
	"Комментарии пользователей"			=> "modules/standart/mobile.UsersComments.php",
);

$JSmodules = array(	
	"Библиотека JQuery"					=> "/modules/standart/js/JQuery.js",
	"Передача данных JsHttpRequest"		=> "/modules/standart/js/JsHttpRequest.js",	
	"Основной JS сайта"  				=> "/modules/standart/js/mobile.MainModule.js",
	"VK комменты и группы" 				=> "http://vk.com/js/api/openapi.js?123",
	#"Авторизация ULogin"				=> "http://ulogin.ru/js/ulogin.js",
	#"Библиотека JQuery.UI"				=> "/modules/standart/js/JQuery.UI.js",
);

$CSSmodules = array(
	"Стандартный Pro.CMS"				=> "/template/standart/mobile.standart.css",
...
if ($data["total"]==0){
	...
	if (is_file("modules/page_mods/mobile.".$node["module"]."-".$node["link"].".php")) { 
			/* PHP */ @header ("HTTP/1.0 200 Ok"); $Robots='<meta name="robots" content="index, follow" />'; require ("modules/page_mods/mobile.".$node["module"]."-".$node["link"].".php");
			/* JS */  if (is_file("modules/page_mods/mobile.".$node["module"]."-".$node["link"].".js")) { $JSmodules[$node["name"]]="/modules/page_mods/mobile.".$node["module"]."-".$node["link"].".js"; }
			/* CSS */ if (is_file("modules/page_mods/mobile.".$node["module"]."-".$node["link"].".css")) { $CSSmodules[$node["name"]]="/modules/page_mods/mobile.".$node["module"]."-".$node["link"].".css"; }
	...
	} elseif (is_file("modules/".$node["module"]."/mobile.".$node["module"].".php")) { 
	...
	if (is_file("modules/page_mods/mobile.".$node["link"].".php") && $Page404!=1) { @require("modules/page_mods/mobile.".$node["link"].".php"); }
...
if (!$GLOBAL["design"]) { 
	$specialdesign=array("index");
	if (in_array($node["module"], $specialdesign) || in_array($node["link"], $specialdesign)) { $design="mainpage"; } else { $design="index"; }
} else {
...
if ($Page["Caption"]!="" && $Page404==0) { $Page["Caption"]="<h1>".nl2br($Page["Caption"])."</h1>"; } if ($node["isindex"]==1) { $Page["Caption"]=""; } $Page["Title"]=Hsc(NormalCaption($Page["Title"]));
...
$RENDER.='<meta name="viewport" content="width=device-width, initial-scale=1.0">'.$r;
